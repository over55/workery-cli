...
func (impl InsuranceRequirementStorerImpl) ListByFilter(ctx context.Context, f *InsuranceRequirementListFilter) (*InsuranceRequirementListResult, error) {
	...
	if f.ExcludeArchived {
		filter["status"] = bson.M{"$ne": InsuranceRequirementStatusArchived} // Do not list archived items! This code
	}
	if f.Status != 0 {
	...
	return &InsuranceRequirementListResult{
		Results:     results,
...
func (impl InsuranceRequirementStorerImpl) ListAsSelectOptionByFilter(ctx context.Context, f *InsuranceRequirementListFilter) ([]*InsuranceRequirementAsSelectOption, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
	defer cancel()
	...
	if f.ExcludeArchived {
		query["status"] = bson.M{"$ne": InsuranceRequirementStatusArchived} // Do not list archived items! This code
	}
