...
type TagPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
type TagPaginationListResult struct {
...
func (impl TagStorerImpl) newPaginationFilter(f *TagPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl TagStorerImpl) newPaginationFilterBasedOnString(f *TagPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl TagStorerImpl) newPaginationFilterBasedOnTimestamp(f *TagPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl TagStorerImpl) newPaginationOptions(f *TagPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl TagStorerImpl) newPaginatorNextCursor(f *TagPaginationListFilter, results []*Tag) (string, error) {
	var lastDatum *Tag
