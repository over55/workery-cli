...
type SkillSetPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
type SkillSetPaginationListResult struct {
...
func (impl SkillSetStorerImpl) newPaginationFilter(f *SkillSetPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
		...
		default:
			return nil, fmt.Errorf("unsupported sort field for `%v`, only supported fields are `sub_category` and `created_at`", f.SortField)
		}
	}
	return bson.M{}, nil
}

func (impl SkillSetStorerImpl) newPaginationFilterBasedOnString(f *SkillSetPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl SkillSetStorerImpl) newPaginationFilterBasedOnTimestamp(f *SkillSetPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl SkillSetStorerImpl) newPaginationOptions(f *SkillSetPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl SkillSetStorerImpl) newPaginatorNextCursor(f *SkillSetPaginationListFilter, results []*SkillSet) (string, error) {
	var lastDatum *SkillSet
	...
	switch f.SortField {
	case "sub_category":
		nextCursor = fmt.Sprintf("%v|%v", lastDatum.SubCategory, lastDatum.ID.Hex())
		break
	case "created_at":
		...
		break
	default:
		return "", fmt.Errorf("unsupported sort field in options for `%v`, only supported fields are `sub_category` and `created_at`", f.SortField)
	}
