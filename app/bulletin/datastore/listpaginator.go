...
func (impl BulletinStorerImpl) newPaginationFilter(f *BulletinPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
		...
		default:
			return nil, fmt.Errorf("unsupported sort field for `%v`, only supported fields are `text` and `created_at`", f.SortField)
		}
	}
	return bson.M{}, nil
}

func (impl BulletinStorerImpl) newPaginationFilterBasedOnString(f *BulletinPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl BulletinStorerImpl) newPaginationFilterBasedOnTimestamp(f *BulletinPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl BulletinStorerImpl) newPaginationOptions(f *BulletinPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl BulletinStorerImpl) newPaginatorNextCursor(f *BulletinPaginationListFilter, results []*Bulletin) (string, error) {
	var lastDatum *Bulletin
