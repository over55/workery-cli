...
func (impl ServiceFeeStorerImpl) ListByFilter(ctx context.Context, f *ServiceFeeListFilter) (*ServiceFeeListResult, error) {
	...
	if f.ExcludeArchived {
		filter["status"] = bson.M{"$ne": ServiceFeeStatusArchived} // Do not list archived items! This code
	}
	if f.Status != 0 {
...
func (impl ServiceFeeStorerImpl) ListAsSelectOptionByFilter(ctx context.Context, f *ServiceFeeListFilter) ([]*ServiceFeeAsSelectOption, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
	defer cancel()
	...
	if f.ExcludeArchived {
		query["status"] = bson.M{"$ne": ServiceFeeStatusArchived} // Do not list archived items! This code
	}
