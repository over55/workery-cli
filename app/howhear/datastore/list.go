...
func (impl HowHearAboutUsItemStorerImpl) ListByFilter(ctx context.Context, f *HowHearAboutUsItemListFilter) (*HowHearAboutUsItemListResult, error) {
	...
	return &HowHearAboutUsItemListResult{
		Results:     results,
...
func (impl HowHearAboutUsItemStorerImpl) ListAsSelectOptionByFilter(ctx context.Context, f *HowHearAboutUsItemListFilter) ([]*HowHearAboutUsItemAsSelectOption, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
	defer cancel()
	...
	if f.ExcludeArchived {
		query["status"] = bson.M{"$ne": HowHearAboutUsItemStatusArchived} // Do not list archived items! This code
	}
